...
export const AssetList: React.FC = () => {
      ...
      render: (item) => (
        <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
          ...
          <Button
            ...
            title="Generate & Print QR Code Tag"
  ...
  return (
    <div className="space-y-6 pb-12 sm:pb-16">
      ...
      {/* Main Inventory Table Card with Non-Scrolling Integrated Footer */}
      <div className={`relative rounded-xl border shadow-card overflow-hidden transition-colors ${
        isLight ? 'bg-white border-slate-200' : 'bg-[#0E1422] border-[#222E3E]'
      }`}>
        ...
        {/* Integrated Asset Inventory Pagination & Footer */}
        <div className={`border-t px-4 py-3 sm:px-5 select-none transition-colors ${
          isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#090D17] border-[#222E3E]'
        }`}>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
            {/* Left Section: Showing Count and Rows Dropdown */}
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs">
              ...
              <div className="flex items-center gap-2">
                <label
                  htmlFor="asset-inventory-rows-select"
                  className={`text-xs font-semibold ${isLight ? 'text-slate-600' : 'text-[#8C9BAE]'}`}
                >
                  Rows:
                </label>
                <div className="relative inline-block">
                  <select
                    id="asset-inventory-rows-select"
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <span className={`px-2 text-xs select-none whitespace-nowrap ${
                isLight ? 'text-slate-600' : 'text-[#8C9BAE]'
              }`}>
                Page <span className={`font-mono font-bold ${isLight ? 'text-slate-900' : 'text-white'}`}>{pagination.page}</span> of{' '}
                <span className={`font-mono font-bold ${isLight ? 'text-slate-900' : 'text-white'}`}>{Math.max(1, pagination.totalPages)}</span>
              </span>
      ...
      {selectedQRAsset && (
        <QRCodeModal
          ...
          assetTitle={selectedQRAsset.assetName || selectedQRAsset.model || 'IT Asset'}
          assetId={selectedQRAsset.id}
        />
      )}
    </div>
  );
