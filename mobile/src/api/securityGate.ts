...
export const securityGateApi = {
  ...
  getCurrentOutside: async (params?: { page?: number; limit?: number; search?: string }): Promise<{ rows: CurrentOutsideItem[]; total: number }> => {
  ...
  getMovements: async (params?: { page?: number; limit?: number; movementType?: 'OUT' | 'IN'; search?: string }): Promise<{ movements: GateMovementRecord[]; total: number }> => {
